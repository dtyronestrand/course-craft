<template>
    <New>
        <div class="bg-base-100 p-12 text-base-content">
            <div class="rounded-t-lg border bg-base-200 p-8">
                <h1 class="mb-4 text-7xl text-base-content">
                    {{ page.props.course.prefix }}
                    {{ page.props.course.number }}
                </h1>
                <h2 class="mb-4 text-5xl text-base-content">
                    {{ page.props.course.title }}
                </h2>
                ...
                <table class="mb-4 w-full table-auto">
                    <thead>
                        <tr>
                            <th class="p-2 text-left">Deliverable</th>
                    ...
                    <tbody>
                        <tr
                            v-for="(deliverable, index) in page.props.course
                                .deliverables"
                            :key="index"
                        >
            ...
            <div class="border bg-base-300 p-8">
                {{ page.props.course.deliverables }}
                <div
                    v-for="(user, index) in page.props.course.users"
                    :key="index"
                >
                    <p>
                        {{ user.pivot.role }}: {{ user.first_name }}
                        {{ user.last_name }}
                    </p>
                </div>
                <h3 class="my-4 text-3xl">Learning Objectives:</h3>
                <div
                    v-for="(objective, index) in page.props.course.objectives"
                    :key="index"
                >
                    <p class="text-lg">
        ...
        <div>
            <component
                :is="currentDisplay"
                :numberOfModules="page.props.numberOfModules"
            ...
            <div v-if="page.props.auth.user?.hasGoogleAccess">
                ...
                >
                    <span v-if="isExporting">Creating Document...</span
                    ><span v-else>Export to Google Drive</span>
                </button>
                <div v-if="exportError" class="mt-2 text-sm text-error">
                    Error: {{ exportError }}
                </div>
                <div v-if="exportSuccessUrl" class="mt-2 text-sm text-success">
            ...
            <div v-else>
                <p class="mb-2 text-sm text-base-content">
                    Connect your Google Drive to export.
                </p>
                <a
                    href="/google/redirect"
                    class="inline-flex items-center bg-info px-4 py-2 text-info-content"
...
import { Delete, Map, Storyboard } from '@/components/CourseActions';
...
const documentTitle = ref(
    `${page.props.course.prefix} ${page.props.course.number} - ${page.props.course.title}`,
);
const documentContent = ref(
    `${page.props.course.objectives.map((obj) => obj.objective).join('\n')}`,
);
const MapComponent = defineAsyncComponent(
    () => import('@/components/Course/Map.vue'),
);
const StoryboardComponent = defineAsyncComponent(
    () => import('@/components/Course/StoryboardComponent.vue'),
);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 401) {
                ...
                exportError.value =
                    data.error || 'Your connection expired. Please reconnect.';
...
const updateDeliverableStatus = (deliverable: any) => {
    router.put(
        `/courses/${page.props.course.id}/deliverables/${deliverable.id}`,
        {
