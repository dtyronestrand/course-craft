<template>
  ...
  >
    ...
    <button class="frosted-backdrop btn bg-info/10 border-info text-info shadow-md shadow-info/10 w-max" @click="addDeliverable">Add Deliverable</button>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr v-for="(deliverable, index) in deliverables" :key="index">
            ...
            <td><button class="btn bg-error/10 text-error border-error  font-bold frosted-backdrop shadow-md shadow-error " @click="() => removeDeliverable(index)">X</button></td>
          </tr>
        </tbody>
      </table>
