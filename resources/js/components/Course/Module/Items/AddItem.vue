...
const selectedItemType = ref<
    'overview' | 'page' | 'quiz' | 'assignment' | 'discussion' | null
>(null);
const emit = defineEmits(['close']);
const itemsMap: Record<string, any> = {
    overview: defineAsyncComponent(
        () =>
            import('@/components/Course/Module/Items/Overview/AddOverview.vue'),
    ),
    page: defineAsyncComponent(
        () => import('@/components/Course/Module/Items/Page/AddPage.vue'),
    ),
    quiz: defineAsyncComponent(
        () => import('@/components/Course/Module/Items/Quiz/AddQuiz.vue'),
    ),
    assignment: defineAsyncComponent(
        () =>
            import(
                '@/components/Course/Module/Items/Assignment/AddAssignment.vue'
            ),
    ),
    discussion: defineAsyncComponent(
        () =>
            import(
                '@/components/Course/Module/Items/Discussion/AddDiscussion.vue'
            ),
    ),
    wrap_up: defineAsyncComponent(
        () => import('@/components/Course/Module/Items/WrapUp/AddWrapUp.vue'),
    ),
};
